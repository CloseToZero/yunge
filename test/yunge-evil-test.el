...
(declare-function evil-set-marker "evil-common"
                  (char &optional pos advance))
...
(ert-deftest yunge-evil-leader-exposes-overridden-marker-command ()
  ...
  (with-temp-buffer
    ...
    (yunge-test-key "SPC j m j" 'yunge-evil-goto-mark)
    (yunge-test-key "SPC j m l" 'yunge-evil-goto-mark-line)
    (save-window-excursion
      (switch-to-buffer (current-buffer))
...
(ert-deftest yunge-evil-centers-after-exact-and-linewise-marker-jumps ()
  (yunge-test-enable-evil)
  (with-temp-buffer
    (insert "zero\n  marked text\nlast\n")
    (goto-char (point-min))
    ...
    (let ((marker-position (point))
      ...
      (save-window-excursion
        (switch-to-buffer (current-buffer))
        (cl-letf (((symbol-function 'evil-scroll-line-to-center)
                   (lambda (count)
                     (should-not count)
                     (cl-incf center-count))))
          (execute-kbd-macro (kbd "' a"))
          (should (= (point) (+ (line-beginning-position) 2)))
          (should (= center-count 1))
          (goto-char (point-max))
          (execute-kbd-macro (kbd "` a"))
          (should (= (point) marker-position))
          (should (= center-count 2)))))))

(ert-deftest yunge-evil-remaps-marker-traversal-to-centering-commands ()
  (yunge-test-enable-evil)
  (with-temp-buffer
    (evil-normal-state)
    (yunge-test-keys
...
(ert-deftest yunge-evil-centers-after-marker-traversal ()
  (yunge-test-enable-evil)
  (let (calls
        (center-count 0))
    (cl-letf (((symbol-function 'evil-next-mark)
               (lambda (count) (push (cons 'next count) calls)))
              ((symbol-function 'evil-next-mark-line)
               (lambda (count) (push (cons 'next-line count) calls)))
              ((symbol-function 'evil-previous-mark)
               (lambda (count) (push (cons 'previous count) calls)))
              ((symbol-function 'evil-previous-mark-line)
               (lambda (count) (push (cons 'previous-line count) calls)))
              ((symbol-function 'evil-scroll-line-to-center)
               (lambda (count)
                 (should-not count)
                 (cl-incf center-count))))
      (yunge-evil-next-mark 2)
      (yunge-evil-next-mark-line 3)
      (yunge-evil-previous-mark 4)
      (yunge-evil-previous-mark-line 5))
    (should
     (equal (nreverse calls)
            '((next . 2) (next-line . 3)
              (previous . 4) (previous-line . 5))))
    (should (= center-count 4))))
