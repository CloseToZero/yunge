...
(ert-deftest yunge-org-character-delete-does-not-preserve-table-width ()
  (yunge-org-delete-test--load-config)
  (dolist (case '(("x" "b")
                  ("X" "c")))
    (with-temp-buffer
      ...
      (let ((width (line-end-position)))
        ...
        (should (= (line-end-position) (1- width)))
        (should-not (string-match-p "b" (buffer-string)))))))

(ert-deftest yunge-org-character-delete-removes-table-padding ()
  (yunge-org-delete-test--load-config)
  (with-temp-buffer
    (org-mode)
    (insert "| A  |\n")
    ...
    (search-forward "A")
    (evil-normal-state)
    (yunge-org-delete-test--with-keys "x x")
