...
(ert-deftest yunge-config-update-coalesces-watches-and-runs-periodically ()
  (let ((yunge-config-directory "C:/config/")
        (yunge-config-update--process nil)
        (yunge-config-update--periodic-timer nil)
        ...
        (yunge-config-update--watching-p nil)
    ...
    (cl-letf
        (((symbol-function 'executable-find)
         ...
         ((symbol-function 'yunge-config-update--current-branch)
          (lambda (_git) "main"))
         ((symbol-function 'yunge-config-update--git-path)
          (lambda (_git path)
            (expand-file-name path "C:/config/.git/")))
         ((symbol-function 'file-exists-p) (lambda (_file) t))
         ((symbol-function 'file-notify-add-watch)
          (lambda (path _flags callback)
            (let ((descriptor (make-symbol "config-watch")))
              (push (list descriptor path callback) watches)
              descriptor)))
         ...
         ((symbol-function 'make-process)
          (lambda (&rest properties)
            ...
            (make-symbol "config-process")))
         ((symbol-function 'process-put) #'ignore)
         ((symbol-function 'process-live-p) #'ignore)
         ((symbol-function 'timerp)
          (lambda (value)
            (or (assq value scheduled) (assq value idle-tasks))))
         ((symbol-function 'run-at-time)
          (lambda (delay repeat function &rest arguments)
            (let ((timer (make-symbol "config-timer")))
              (push (list timer delay repeat function arguments) scheduled)
              timer)))
         ((symbol-function 'run-with-idle-timer)
          (lambda (delay _repeat function &rest arguments)
            (let ((timer (make-symbol "config-idle-timer")))
              (push (list timer delay function arguments) idle-tasks)
              timer)))
         ((symbol-function 'cancel-timer)
          (lambda (timer) (push timer cancelled))))
      (unwind-protect
          (progn
            (yunge-config-update-start)
            (should (= (length watches) 2))
            (should (= (length commands) 1))

            (pcase-let ((`(,descriptor ,path ,callback) (car watches)))
              (funcall callback (list descriptor 'changed path))
              (funcall callback (list descriptor 'changed path)))
            (let ((debounce
                   (seq-find
                    (lambda (task)
                      (and (null (nth 2 task))
                           (not (memq (car task) cancelled))))
                    scheduled)))
              (should debounce)
              (apply (nth 3 debounce) (nth 4 debounce)))
            (should
             (= (cl-count-if
                 (lambda (command) (member "rev-list" command))
                 commands)
                1))

            (let ((periodic
                   (seq-find (lambda (task) (nth 2 task)) scheduled)))
              (should periodic)
              (apply (nth 3 periodic) (nth 4 periodic)))
            (should (= (length idle-tasks) 1))
            (apply (nth 2 (car idle-tasks)) (nth 3 (car idle-tasks)))
            (should
             (= (cl-count-if
                 (lambda (command) (member "fetch" command))
                 commands)
                2))
        ...
        (dolist (buffer (buffer-list))
          (when (string-prefix-p " *yunge-config-update-"
                                 (buffer-name buffer))
            (kill-buffer buffer)))))))
