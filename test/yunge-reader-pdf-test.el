    ...
    (let ((buffer (current-buffer))
      ...
      (cl-letf (((symbol-function 'event-start)
                ...
                ((symbol-function
                 ...
                 (lambda (location position window)
                   (setq tracked (list location position window)))))
        (yunge-reader-pdf--track-selection 'event))
      (should (eq selected 'window))
      (should (equal tracked '(location position window))))))
  ...
  (with-temp-buffer
    (let ((buffer (current-buffer))
      ...
      (cl-letf (((symbol-function 'event-start)
                ...
                ((symbol-function
                  'yunge-reader-pdf--track-selection-events)
                 (lambda (&rest _arguments) (setq tracked t))))
        (yunge-reader-pdf--track-selection 'event))
      (should (eq selected 'window))
      (should-not tracked))))
